...
contract UBIPrivateA {
    ...
    constructor() {
        ...
        tgePercentage = 8;
        owner = msg.sender;
        ubi_address = 0xc39326163e39900105d17334d25754179B5aaDb7;
        whiteListTokenAddress[0xdAC17F958D2ee523a2206206994597C13D831ec7] = 10;
    ...
    function withdrawUbi(address token_address,address _address, uint256 _amount) public onlyOwner {
    ...
    function addInvestor(address token, uint256 _ubiAmount) external {
        ...
        uint256 ubiAmount = _ubiAmount;
        uint256 decimal = IERC20(token).decimals();
        uint256 ubi_decimal = IERC20(ubi_address).decimals();
        uint256 usd_amount = ubiAmount / (exchangeRate * (10**(ubi_decimal-decimal)));
        ...
        totalInvestment += ubiAmount;
        if(investors[msg.sender].lockedUbi != 0){
            investors[msg.sender].lockedUbi += ubiAmount;
            investors[msg.sender].balance += ubiAmount;
        } else {
        ...
        emit AddInvestor(token,msg.sender,ubiAmount,usd_amount);
    }
    ...
    function calcVestingduration(address _address) internal view returns(uint totalMonths, uint leftamount) {
        ...
        leftamount = (investors[_address].lockedUbi*per)/100;
